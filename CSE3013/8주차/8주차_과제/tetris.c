...
void newBlockDown(int sig){//새롭게 정의된 함수로 1초마다 실행을 위한 함수이다.
	int chk;
	int i,j;
	time_t start,stop;
	if(CheckToMove(field,nextBlock[0],blockRotate,blockY,blockX))
	{	//gaemove가 아니라면 필드에 추가, 라인삭제를 진행한다.
		gameOver==0;
		if(gameOver==0){	
			score+=10*AddBlockToField(field,nextBlock[0],max.bR,max.bY,max.bX);
			blockY=max.bY;blockX=max.bX,blockRotate=max.bR;	
			score+=DeleteLine(field);
		}//필드를 그리고, 점수를 그린다.
                DrawField();
                PrintScore(score);//게임정보를 초기화한다.
		for(i=0;i<BLOCK_NUM-1;i++){
			nextBlock[i]=nextBlock[i+1];
		}
		nextBlock[BLOCK_NUM-1]=rand()%7;
		...
		blockRotate=0;blockY=-1;blockX=WIDTH/2-2;
		makingroot();
		max.score=-10000;//추천시작
		start=time(NULL);
		modified_recommend(Recroot);
		stop=time(NULL);
		duration+=(double)difftime(stop,start);
		memory+=evalSize(Recroot);
		if(max.score==-10000){//추첮실패
			gameOver=1;
		}
		else//추천블록그리기
			DrawBlock(max.bY,max.bX,nextBlock[0],max.bR,'R');
	...
	move(HEIGHT,WIDTH+10);
	printw("memory for rec: %ld b",memory);
	move(HEIGHT+1,WIDTH+10);
	printw("time for rec : %lf sec",duration);
	timed_out=0;
}
	...
	for (i = 3; i >=0 ; i--) {
		for (j = 0; j < 4; j++) {
			if (block[currentBlock][blockRotate][i][j] == 1) {
				...
				for(t=HEIGHT-1;t<blockY+i;t--){
					if(field[t][blockX+j==0])
						bigminus+=1;
				}
			}
...
RR modified_recommend(RecNode* root) {
	RR tem, loop; // 미리 보이는 블럭의 추천 배치까지 고려했을 때 얻을 수 있는 최대 점수
	RecNode* prev = NULL, * curr = root;
	tem.score = -10000;
	int t,bests=-1000;
	...
	makingnode(root, nextBlock[(root->lv)]);
//노드 원소중 최고의 점수를 가진 점수를 찾는다. 바뀐부분은 이것밖에 없다.
	for (t = 0; t < root->c[0]->howmanycase; t++){ 
		bests = (bests > root->c[t]->score) ? bests : root->c[t]->score;
	}

	for (t = 0; t < root->c[0]->howmanycase; t++) {
		if (root->c[t] && root->c[t]->valid==1) {
			//최고의 점수 보다 최대 100낮은것이 혀용범위. 이때만 탐색을 한다.
			if (root->c[t]->score > bests - 400) {
				loop = recommend(root->c[t]);
				max = (max.score > loop.score) ? max : loop;
			}
			else//그 외엔 의미없는 자식으로 연결을 끊을준비를 한다.
			{
				root->c[t]->valid=0;
			}
		}
...
//자식이 없다면 메모리 없음
	...
	{	//그외 노드의 메모리 재귀적으로 return.
		for (k = 0; k < aa->c[0]->howmanycase; k++) {
			if ((aa->c[k] !=NULL) && aa->c[k]->valid == 1) {
				return sizeof(RecNode) + evalSize(aa->c[k]);
			}
		}
	}
}
