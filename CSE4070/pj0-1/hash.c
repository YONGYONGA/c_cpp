...
bool pj_less_func(const struct hash_elem *a,const struct hash_elem *b,void *aux){
	struct hash_item *one=hash_entry(a,struct hash_item,he);
	struct hash_item *two=hash_entry(b,struct hash_item,he);
	if(one->key<two->key)
		return true;
	else
		return false;
}
unsigned pj_hash_func(const struct hash_elem *a,void *aux){
	struct hash_item *one=hash_entry(a,struct hash_item,he);
	return hash_int(one->key);
}
void square(struct hash_elem *a,void *aux){
	struct hash_item *data=hash_entry(a,struct hash_item,he);
	(data->key)*=(data->key);
}
...
void destructor(struct hash_elem *d,void*aux){
	free(d);
}
