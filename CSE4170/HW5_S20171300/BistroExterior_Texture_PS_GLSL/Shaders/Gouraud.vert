...
vec4 lighting_equation(in vec3 P_EC, in vec3 N_EC) {
	...
	color_sum = u_material.emissive_color + u_global_ambient_color * u_material.diffuse_color;
	...
	for (int i = 0; i < NUMBER_OF_LIGHTS_SUPPORTED; i++) {
		...
		if (local_scale_factor > zero_f) {				
			vec4 local_color_sum = u_light[i].ambient_color * u_material.diffuse_color;
